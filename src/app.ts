...
/** Complete guild-command payload. Kept pure so a pre-extraction golden can
 * prove that moving command ownership does not change Discord registration. */
...
export class DiscordCopilotApp {
  ...
  private async registerCommands(clientId: string): Promise<void> {
    ...
    await rest.put(Routes.applicationGuildCommands(clientId, this.config.DISCORD_GUILD_ID), {
      body: buildCommandRegistrationPayload(this.modelIds),
    });
  }
