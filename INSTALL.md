# discopilot — 安裝指南 / Installation Guide

> ⚠️ **僅限實驗環境 (v1) / LAB-ONLY (v1)**
> discopilot 讓 Discord 直接控制你本機的 GitHub Copilot：agent 會**以你的身分執行 shell 指令並修改檔案**，v1 沒有隔離。請**只**在可拋棄的 VM／測試帳號／測試 repo 上使用。
> discopilot lets Discord drive your local GitHub Copilot: the agent **runs shell commands and edits files as you**, with no isolation in v1. Use **only** on a disposable VM / test account / throwaway repo.
...
```bash
git clone https://github.com/lettucebo/discopilot.git
cd discopilot
```
...
- **macOS**：`~/Library/LaunchAgents/com.discopilot.<instance>.plist`
- **Linux**：`~/.config/systemd/user/discopilot-<instance>.service`（登入前常駐：`loginctl enable-linger $USER`）

多重部署 / Multiple deployments：設定 `DISCOPILOT_INSTANCE_ID`（預設 `default`），常駐資源名稱會隨之改變。
...
- `.env` 備份存放於 `~/.discopilot/env-backups/`（權限僅限本人）。/ `.env` backups live under `~/.discopilot/env-backups/` (owner-only).
