...
- **A thread stopped responding after a permission or channel change (`thread-no-access`)** → this is **retryable**: restore the bot's access to that channel and the running bot resumes the session by itself, without a restart (it rescans such records every 15s, backing off to at most every 5 minutes). `/sessions` lists it separately from other records; if you want it gone instead, `/end thread:<id>` clears it explicitly. See [`docs/CHANNEL-ACCESS.md`](docs/CHANNEL-ACCESS.md).
