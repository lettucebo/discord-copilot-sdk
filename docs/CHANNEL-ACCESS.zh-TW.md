...
> 如果 bot 意外失去它自己在某頻道的 View Channel（有人改了權限、身分組變動等），對該頻道的 API 呼叫——包括綁定在它上面的既有 session——都會得到 `50001 Missing Access`。依 [ADR-0002](adr/0002-missing-access-is-retryable.md)，這個特定失敗被視為**可重試**，不是終態：復原 bot 的存取權後，受影響的 session 就會恢復，這和 §7 描述的結構性不符不同。恢復**不需要重啟**——執行中的 bot 會定期重掃它的 `thread-no-access` 紀錄（第一次 15 秒後，之後逐步退避到最多每 5 分鐘一次；沒有東西要復原時就固定用最長的間隔輪詢），一旦該討論串重新判定為可存取就復原它；重啟只是把同一個檢查立刻跑一次而已。重新取得存取權時，Discord 確實會對**該頻道**送出 `CHANNEL_UPDATE`（見 §7a），可以刷新 parent 與 cache——但那個事件並不會告訴你**哪些綁定的討論串**現在可以復原了，而討論串是另一個 channel 物件，可能從來沒被 cache、也不會收到對應的更新；所以它是有用的提示，不是充分的正確性來源。這就是為什麼機制是週期掃描，也是為什麼每次重試都用 `force` 重新抓取討論串，而不是相信一個對它而言只存著混淆 stub 的 cache。`/sessions` 會把這類 `thread-no-access` 紀錄列在專屬的區段——存取權恢復後會自動重試，但擁有者也可以刻意放棄它、用 `/end thread:<id>` 明確清除。
