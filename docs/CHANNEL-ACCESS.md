...
> If the bot loses its own View Channel on a channel unexpectedly (someone edits permissions, a role change, etc.), API calls against that channel — including for existing sessions bound to it — fail with `50001 Missing Access`. Per [ADR-0002](adr/0002-missing-access-is-retryable.md), that specific failure is treated as **retryable**, not terminal: restoring the bot's access lets the affected sessions resume, unlike the structural mismatches described in §7. Recovery does **not** need a restart — the running bot re-checks its `thread-no-access` records on a periodic scan (first after 15s, backing off to at most once every 5 minutes) and resumes each one as soon as the thread classifies as reachable again; restarting simply runs the same check immediately. Restoring access does not reliably produce a gateway event about a thread the bot could not see, which is why the scan, not an event, is what makes this promise true. `/sessions` lists such `thread-no-access` records in their own section — retried automatically after access is restored, but the owner can also deliberately abandon one by clearing it with `/end thread:<id>`.
